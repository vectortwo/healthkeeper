...
public class FitDoughnut extends ViewGroup {
    ...
    private static final long ANIM_DURATION_DEFAULT = 1500;
    private static final long ANIM_DURATION_LONG = 4000;
    ...
    public FitDoughnut(Context ctx, AttributeSet attrs) {
        ...
        try {
            ...
            colorTextPrimary = a.getColor(R.styleable.FitDoughnut_fdColorTextPrimary, Color.rgb(225, 140, 80));
            colorTextSecondary = a.getColor(R.styleable.FitDoughnut_fdColorTextSecondary, Color.rgb(200,200,200));
            text = a.getString(R.styleable.FitDoughnut_fdText);
            float _textSize = a.getFloat(R.styleable.FitDoughnut_fdTextSize, 18);//a.getFloat(R.styleable.FitDoughnut_fdText, 100.f);
            textSize = convertSpToPixels(_textSize, ctx);
        } finally {
    ...
    private void init() {
        ...
        paintTextPrimary.setTextSize(textSize);
    ...
    public void animateSetTime(int time){
        float old = getPercent();
        setPercent((float)time/60.f * 100.f);
        setText(String.valueOf(time)+"min");
        animateRing(old, getPercent(), FitDoughnut.ANIM_DURATION_DEFAULT);
    }

    public void animateSetPercent(int time, int endTime){
        float old = getPercent();
        float percent = (float)time/(float)endTime*100.0f;
        setPercent(percent);
        setText(String.valueOf((int)percent)+"%");
        animateRing(old, getPercent(), FitDoughnut.ANIM_DURATION_DEFAULT/3);
    }
    ...
    private static float convertSpToPixels(float sp, Context context) {
    ...
    class FitDoughnutView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            paintTextPrimary.getTextBounds(text, 0, text.length(), bounds);
            canvas.drawText(text, oval.centerX()-bounds.centerX(), oval.centerY()+(bounds.height()/2), paintTextPrimary);
            ...
            //canvas.drawRect(oval.right*1, oval.top/2, oval.right*4, oval.top/4, paintPrimary);
            //float x = (oval.centerX()+bounds.centerX());
            //float y = (oval.centerY()+(bounds.height()/4));
            //canvas.drawText(text, x, y, paintTextPrimary);
        }
